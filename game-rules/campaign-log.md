# 📜 LEGO QUEST — Campaign Log

*Track sessions, quests, loot, and the story as it unfolds. v1.1 Rebalance*

---

## Campaign Overview

| Field | Value |
|-------|-------|
| **Campaign Name** | *TBD* |
| **Start Date** | *TBD* |
| **Sessions Played** | 0 |
| **Current Quest** | The Stolen Crown (Level 1) |
| **Party Level** | 1 |
| **World Threat** | The Shadow Tower (teased) |
| **Rules Version** | v1.1 (Early Level Rebalance) |

---

## Quest Tracker

| Quest | Level | Status | Reward | Session |
|-------|-------|--------|--------|---------|
| The Stolen Crown | 1 | 🟡 Not Started | 15 XP + 10 gold + seals | — |
| The Shadow Tower | 2–3 | 🔒 Locked | *TBD* | — |
| *Future quests...* | — | 🔒 Locked | — | — |

---

## Session Log

### Session 1 — *Date TBD*

**Adventure:** The Stolen Crown
**Duration:** ~mins
**Summary:** *TBD*
**Special Rules:** Heroic Start (advantage until first combat victory)

#### XP Earned

| Activity | Hero 1 | Hero 2 |
|----------|--------|--------|
| Combat | — | — |
| Quest | — | — |
| Puzzles | — | — |
| Creativity Bonus | — | — |
| Session Bonus | — | — |
| **TOTAL** | **—** | **—** |

#### Loot Collected

| Item/Seal | Hero 1 | Hero 2 |
|-----------|--------|--------|
| Gold | — | — |
| Common Seals | — | — |
| Uncommon Seals | — | — |
| Items | — | — |

#### Key Moments

- *TBD*

#### Level Ups

- *TBD*

#### Skill Tree Choices

- *TBD*

---

*Copy the Session template above for each new session*

---

## World Notes

*Track NPCs, locations, story threads, and open mysteries.*

### Known NPCs

| Name | Role | Location | Status |
|------|------|----------|--------|
| The Mayor | Quest Giver | Brickton | Active |
| The Goblin King | Villain | The Old Mine | Active |
| *Shopkeeper TBD* | Merchant | Brickton | Active |

### Known Locations

| Location | Level Range | Status | Notes |
|----------|------------|--------|-------|
| Brickton Village | Hub | ✅ Discovered | Home base, shop, Mayor |
| The Forest Path | 1 | 🟡 Upcoming | Giant rats (minions!), fallen tree, hidden chest |
| The Old Mine | 1 | 🟡 Upcoming | 3 rooms, Goblin King boss (HP 16, Dmg 4) |
| The Shadow Tower | 2–3 | 🔒 Locked | Teased at end of quest 1 |

### Story Threads

| Thread | Status | Notes |
|--------|--------|-------|
| Who controls the Goblin King? | 🔴 Open | Revealed after quest 1 |
| The Shadow Tower | 🔴 Open | Visible on the horizon |
| The Magic Gate | 🔴 Open | Crown powers the village protection |

### Lego Build Inventory

*Track what physical Lego builds you've made for the game.*

| Build | Status | Notes |
|-------|--------|-------|
| Brickton Village | ⬜ Not built | 4–5 buildings |
| Forest Path | ⬜ Not built | Green plates + trees |
| Mine Entrance | ⬜ Not built | Gray bricks, 3 rooms |
| Goblin King Throne | ⬜ Not built | Boss room centerpiece |

### Physical Token Inventory

*Track Lego pieces used as game tokens*

| Token | Lego Piece | Qty Available |
|-------|-----------|---------------|
| Common Seal 🟢 | Green 1×1 flat tile | *count* |
| Uncommon Seal 🔵 | Blue stud | *count* |
| Gold Coins | Yellow 1×1 round | *count* |
| Hero Surge Token | Special 1×1 tile (unique) | 2 (one per hero) |
