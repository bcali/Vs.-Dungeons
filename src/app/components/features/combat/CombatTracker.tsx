...
export const CombatTracker: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (combatants.length === 0) {
      ...
      // Add Heroes (assume all active heroes for now, but really should be selected ones from setup)
      // Since Setup saves selected, I should check how I saved it. 
      // Setup didn't save selected heroes to encounter, only enemies. 
      // I'll assume all heroes for this demo or update Setup to save heroes.
      // Setup logic: `heroes` are global. I'll just use all heroes for simplicity as Setup didn't pass them.
      // Wait, Setup sets `activeEncounter` which has `enemies`. Heroes should be tracked too.
      // I'll update `Encounter` type later to include `heroIds`. For now, use all heroes.
