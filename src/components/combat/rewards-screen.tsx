  ...
  if (loading) {
    return (
      <div className="min-h-screen p-6 flex items-center justify-center">
        <span className="text-text-muted animate-pulse">Calculating rewards...</span>
      </div>
    );
  }
          ...
          <div className="col-span-4 space-y-4 overflow-y-auto">
            <div className="rounded-xl bg-bg-card border border-border-card p-4">
              ...
              {summary.enemiesDefeated.length === 0 ? (
                ...
                <div className="space-y-1">
                  {summary.enemiesDefeated.map((e, i) => (
                    <div
                      key={i}
                      className="flex justify-between text-sm rounded-lg bg-bg-input/50 px-3 py-2"
                    >
                      <span>
                        {e.name}
                        {e.isBoss && (
                          <span className="text-accent-gold text-xs ml-1">
                            BOSS
                          </span>
                        )}
                      </span>
                      <span className="text-accent-gold">{e.xpReward} XP</span>
                    </div>
                  ))}
                </div>
              )}
              ...
              {summary.enemiesSurvived.length > 0 && (
                <>
                  <h3 className="text-xs text-text-muted mt-3 mb-1 uppercase">
                    Survived (0 XP)
                  </h3>
                  {summary.enemiesSurvived.map((e, i) => (
                    <div
                      key={i}
                      className="text-sm text-text-dim px-3 py-1"
                    >
                      {e.name}
                    </div>
                  ))}
              ...
              <div className="mt-4 pt-3 border-t border-border-card space-y-1">
            ...
            <div className="rounded-xl bg-bg-card border border-border-card p-4">
              ...
              <input
                ...
                className="w-full rounded-lg bg-bg-input px-3 py-2 text-sm text-white placeholder-zinc-500 mb-2"
              />
              ...
              <div className="space-y-1 max-h-48 overflow-y-auto">
                {filteredCatalog.map((item) => (
                  <button
                    ...
                    className="w-full text-left rounded-lg bg-bg-input/50 px-3 py-2 text-sm hover:bg-bg-input transition-colors disabled:opacity-40"
                  >
                ...
                {filteredCatalog.length === 0 && (
                  <p className="text-xs text-text-dim italic">No items found</p>
                )}
              </div>

              {/* Custom item */}
              <div className="mt-3 pt-3 border-t border-border-card">
                <p className="text-xs text-text-secondary mb-1">Custom Item</p>
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 rounded-lg bg-bg-input px-2 py-1.5 text-xs text-white placeholder-zinc-500"
                    disabled={applied}
                  />
                  <select
                    value={customItemType}
                    onChange={(e) => setCustomItemType(e.target.value)}
                    className="rounded-lg bg-bg-input px-2 py-1.5 text-xs text-white"
                    disabled={applied}
                  >
                  ...
                  <button
                    ...
                    className="rounded-lg bg-accent-gold px-3 py-1.5 text-xs font-bold text-bg-page hover:bg-accent-gold/80 disabled:opacity-40"
                  >
                    Add
                  </button>
                </div>
            ...
            {heroStates.map((hero) => {
              ...
              return (
                <div
                  ...
                  className={`rounded-xl bg-bg-card border p-4 cursor-pointer transition-colors ${
                    isActive
                      ? "border-accent-gold"
                      : "border-border-card hover:border-zinc-500"
                  }`}
                >
                  <div className="flex items-start justify-between mb-3">
                    <div>
                      ...
                      <p className="text-xs text-text-secondary">
                        ...
                        {levelsGained > 0 && (
                          <span className="text-accent-gold font-bold ml-2 animate-pulse">
                            → Level {newLevel}!
                          </span>
                        )}
                      </p>
                    ...
                    {levelsGained > 0 && (
                      <span className="bg-accent-gold/20 text-accent-gold text-xs font-bold px-3 py-1 rounded-full animate-pulse">
                        LEVEL UP!
                      </span>
                    )}
                  </div>
                  ...
                  <div className="mb-3">
                    ...
                    <div className="w-full h-3 bg-bg-input rounded-full relative overflow-hidden">
                      <div
                        className="absolute h-full bg-accent-gold/30 rounded-full"
                  ...
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <input
                      ...
                      placeholder="0"
                      className="w-20 rounded-lg bg-bg-input px-2 py-1 text-sm text-accent-gold text-center"
                      disabled={applied}
                    />
                    {hero.goldToAdd > 0 && (
                  ...
                  {hero.items.length > 0 && (
                    <div>
                      <p className="text-xs text-text-secondary mb-1">Items to receive:</p>
                      <div className="flex flex-wrap gap-1">
                        {hero.items.map((item, idx) => (
                          <span
                            key={idx}
                            className="inline-flex items-center gap-1 bg-bg-input rounded-lg px-2 py-1 text-xs"
                          >
                            {item.itemName}
                            {!applied && (
                              <button
              ...
              <button
                ...
                className={`rounded-xl px-8 py-3 text-lg font-bold transition-colors ${
                  applied
                    ...
                    : "bg-accent-gold text-bg-page hover:bg-accent-gold/80"
                }`}
              >
                {applied
                  ? "Rewards Applied!"
